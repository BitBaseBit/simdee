...
#include "../types/jsimd_base.hpp"
#include "jsimd_types_include.hpp"

#undef JSIMD_BATCH_INT_SIZE
#undef JSIMD_BATCH_FLOAT_SIZE
#undef JSIMD_BATCH_DOUBLE_SIZE

#if JSIMD_X86_INSTR_SET >= JSIMD_X86_AVX512_VERSION
#define JSIMD_BATCH_INT8_SIZE 64
#define JSIMD_BATCH_INT16_SIZE 32
#define JSIMD_BATCH_INT32_SIZE 16
#define JSIMD_BATCH_INT64_SIZE 8
#define JSIMD_BATCH_FLOAT_SIZE 16
#define JSIMD_BATCH_DOUBLE_SIZE 8
#elif JSIMD_X86_INSTR_SET >= JSIMD_X86_AVX_VERSION
#define JSIMD_BATCH_INT8_SIZE 32
#define JSIMD_BATCH_INT16_SIZE 16
#define JSIMD_BATCH_INT32_SIZE 8
#define JSIMD_BATCH_INT64_SIZE 4
#define JSIMD_BATCH_FLOAT_SIZE 8
#define JSIMD_BATCH_DOUBLE_SIZE 4
#elif JSIMD_X86_INSTR_SET >= JSIMD_X86_SSE2_VERSION
#define JSIMD_BATCH_INT8_SIZE 16
#define JSIMD_BATCH_INT16_SIZE 8
#define JSIMD_BATCH_INT32_SIZE 4
#define JSIMD_BATCH_INT64_SIZE 2
#define JSIMD_BATCH_FLOAT_SIZE 4
#define JSIMD_BATCH_DOUBLE_SIZE 2
...
#define JSIMD_BATCH_DOUBLE_SIZE 1
#endif
#endif
...
namespace jsimd
{
...
#ifdef JSIMD_BATCH_FLOAT_SIZE
    template <>
    struct simd_traits<int8_t>
    {
        using type = batch<int8_t, JSIMD_BATCH_INT8_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<int8_t, JSIMD_BATCH_INT8_SIZE>>
    {
        using type = int8_t;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    {
        using type = batch<uint8_t, JSIMD_BATCH_INT8_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<uint8_t, JSIMD_BATCH_INT8_SIZE>>
    {
        using type = uint8_t;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    {
        using type = batch<int16_t, JSIMD_BATCH_INT16_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<int16_t, JSIMD_BATCH_INT16_SIZE>>
    {
        using type = int16_t;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    {
        using type = batch<uint16_t, JSIMD_BATCH_INT16_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<uint16_t, JSIMD_BATCH_INT16_SIZE>>
    {
        using type = uint16_t;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    {
        using type = batch<int32_t, JSIMD_BATCH_INT32_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<int32_t, JSIMD_BATCH_INT32_SIZE>>
    {
        using type = int32_t;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    {
        using type = batch<uint32_t, JSIMD_BATCH_INT32_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<uint32_t, JSIMD_BATCH_INT32_SIZE>>
    {
        using type = uint32_t;
        static constexpr size_t size = simd_traits<type>::size;
    };
...
// On some architectures long is a different type from int32_t or int64_t
    ...
    template <>
    struct revert_simd_traits<batch<long, JSIMD_BATCH_INT32_SIZE>>
    {
        using type = long;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    template <>
    struct revert_simd_traits<batch<unsigned long, JSIMD_BATCH_INT32_SIZE>>
    {
    ...
    {
        using type = batch<int64_t, JSIMD_BATCH_INT64_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<int64_t, JSIMD_BATCH_INT64_SIZE>>
    {
        using type = int64_t;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    {
        using type = batch<uint64_t, JSIMD_BATCH_INT64_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<uint64_t, JSIMD_BATCH_INT64_SIZE>>
    {
        using type = uint64_t;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    {
        using type = batch<float, JSIMD_BATCH_FLOAT_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<float, JSIMD_BATCH_FLOAT_SIZE>>
    {
        using type = float;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    {
        using type = batch<std::complex<float>, JSIMD_BATCH_FLOAT_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<std::complex<float>, JSIMD_BATCH_FLOAT_SIZE>>
    {
    ...
    {
        using type = batch<xtl::xcomplex<float, float, i3ec>, JSIMD_BATCH_FLOAT_SIZE>;
        using bool_type = typename simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <bool i3ec>
    struct revert_simd_traits<batch<xtl::xcomplex<float, float, i3ec>, JSIMD_BATCH_FLOAT_SIZE>>
    {
        using type = xtl::xcomplex<float, float, i3ec>;
        static constexpr size_t size = simd_traits<type>::size;
    };
#endif // JSIMD_ENABLE_XTL_COMPLEX
#endif // JSIMD_BATCH_FLOAT_SIZE

#ifdef JSIMD_BATCH_DOUBLE_SIZE
    template <>
    struct simd_traits<double>
    {
        using type = batch<double, JSIMD_BATCH_DOUBLE_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<double, JSIMD_BATCH_DOUBLE_SIZE>>
    {
        using type = double;
        static constexpr size_t size = simd_traits<type>::size;
    };
    ...
    {
        using type = batch<std::complex<double>, JSIMD_BATCH_DOUBLE_SIZE>;
        using bool_type = simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <>
    struct revert_simd_traits<batch<std::complex<double>, JSIMD_BATCH_DOUBLE_SIZE>>
    {
    ...
    {
        using type = batch<xtl::xcomplex<double, double, i3ec>, JSIMD_BATCH_DOUBLE_SIZE>;
        using bool_type = typename simd_batch_traits<type>::batch_bool_type;
        static constexpr size_t size = type::size;
    };

    template <bool i3ec>
    struct revert_simd_traits<batch<xtl::xcomplex<double, double, i3ec>, JSIMD_BATCH_DOUBLE_SIZE>>
    {
        using type = xtl::xcomplex<double, double, i3ec>;
        static constexpr size_t size = simd_traits<type>::size;
    };
#endif // JSIMD_ENABLE_XTL_COMPLEX
#endif // JSIMD_BATCH_DOUBLE_SIZE
