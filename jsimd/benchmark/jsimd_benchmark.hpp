#ifndef JSIMD_BENCHMARK_HPP
#define JSIMD_BENCHMARK_HPP
...
namespace jsimd
{
    ...
    template <class T>
    using bench_vector = std::vector<T, jsimd::aligned_allocator<T, JSIMD_DEFAULT_ALIGNMENT>>;
...
#define DEFINE_FUNCTOR_1OP(FN)\
    struct FN##_fn {\
        template <class T>\
        inline T operator()(const T& x) const { using jsimd::FN; return FN(x); }\
...
#define DEFINE_FUNCTOR_1OP_TEMPLATE(FN, N, ...)\
    struct FN##_##N##_fn {\
        template <class T>\
        inline T operator()(const T& x) const { using jsimd::FN; return FN<T, __VA_ARGS__>(x); }\
...
#define DEFINE_FUNCTOR_2OP(FN)\
    struct FN##_fn{\
        template <class T>\
        inline T operator()(const T&lhs, const T& rhs) const { using jsimd::FN; return FN(lhs, rhs); }\
        inline std::string name() const { return #FN; }\
    }
...
#define DEFINE_FUNCTOR_3OP(FN)\
    struct FN##_fn{\
        template <class T>\
        inline T operator()(const T& op0, const T& op1, const T& op2) const { using jsimd::FN; return FN(op0, op1, op2); }\
        inline std::string name() const { return #FN; }\
    }
