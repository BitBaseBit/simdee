...
namespace JVec {
    ...
    JVec_inline vec2f operator-(const vec2f& lhs) {
        return vec2f( simd4f_sub(simd4f_zero(), lhs.value) );
    }


    JVec_inline vec2f operator+(const vec2f& lhs, const vec2f& rhs) {
    ...
    JVec_inline vec2f operator+=(vec2f& lhs, const vec2f& rhs) {
    ...
    JVec_inline vec2f operator+(const vec2f& lhs, float rhs) {
    ...
    JVec_inline vec2f operator+(float lhs, const vec2f& rhs) {
    ...
    JVec_inline vec2f operator+=(vec2f& lhs, float rhs) {
    ...
    JVec_inline float dot(const vec2f& lhs, const vec2f& rhs) {
        return simd4f_get_x( simd4f_dot2(lhs.value, rhs.value) );
    }

    
    JVec_inline float length(const vec2f& v) {
        return simd4f_get_x( simd4f_length2(v.value) );
    }

    JVec_inline float length_squared(const vec2f& v) {
        return simd4f_get_x( simd4f_length2_squared(v.value) );
    }

    JVec_inline vec2f normalize(const vec2f& v) {
        return vec2f( simd4f_normalize2(v.value) );
    }

    JVec_inline vec2f min(const vec2f& a, const vec2f& b) {
        return vec2f( simd4f_min(a.value, b.value) );
    }

    JVec_inline vec2f max(const vec2f& a, const vec2f& b) {
...
namespace std {
    inline ::JVec::vec2f min(const ::JVec::vec2f& a, const ::JVec::vec2f& b) { return ::JVec::min(a,b); }
    inline ::JVec::vec2f max(const ::JVec::vec2f& a, const ::JVec::vec2f& b) { return ::JVec::max(a,b); }
}
...
JVec_inline std::ostream& operator<<(std::ostream& os, const JVec::vec2f& v) {
    os << "[ " << v.x() << ", "
               << v.y() << " ]";
    return os;
}
