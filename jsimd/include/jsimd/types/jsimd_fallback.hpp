...
namespace jsimd
{
    ...
    struct simd_batch_traits<batch_bool<T, N>>
    {
        ...
        static constexpr std::size_t align = JSIMD_DEFAULT_ALIGNMENT;
    };
    ...
    public:
        ...
        JSIMD_DECLARE_LOAD_STORE_ALL(T, N)
    ...
    struct simd_batch_traits<batch_bool<std::complex<T>, N>>
        : complex_batch_bool_traits<std::complex<T>, T, N, JSIMD_DEFAULT_ALIGNMENT>
    {
    };
    ...
    struct simd_batch_traits<batch<std::complex<T>, N>>
        : complex_batch_traits<std::complex<T>, T, N, JSIMD_DEFAULT_ALIGNMENT>
    {
    };
    ...
    struct simd_batch_traits<batch_bool<xtl::xcomplex<T, T, i3ec>, N>>
        : complex_batch_bool_traits<xtl::xcomplex<T, T, i3ec>, T, N, JSIMD_DEFAULT_ALIGNMENT>
    {
    };
    ...
    struct simd_batch_traits<batch<xtl::xcomplex<T, T, i3ec>, N>>
        : complex_batch_traits<xtl::xcomplex<T, T, i3ec>, T, N, JSIMD_DEFAULT_ALIGNMENT>
    {
    };
...
#define JSIMD_FALLBACK_MAPPING_LOOP(RESULT_TYPE, EXPRESSION)  \
...
#define JSIMD_FALLBACK_UNARY_OP(RESULT_TYPE, OPERATOR, X)  \
    JSIMD_FALLBACK_MAPPING_LOOP(RESULT_TYPE, (OPERATOR X[i]))

#define JSIMD_FALLBACK_BINARY_OP(RESULT_TYPE, OPERATOR, X, Y)  \
    JSIMD_FALLBACK_MAPPING_LOOP(RESULT_TYPE, (X[i] OPERATOR Y[i]))

#define JSIMD_FALLBACK_BATCH_BITWISE_UNARY_OP(OPERATOR, X)  \
    JSIMD_FALLBACK_MAPPING_LOOP(  \
        batch,  \
...
#define JSIMD_FALLBACK_BATCH_BITWISE_BINARY_OP(OPERATOR, X, Y)  \
    JSIMD_FALLBACK_MAPPING_LOOP(  \
        batch,  \
        detail::from_unsigned_integer<T>(  \
...
#define JSIMD_FALLBACK_BATCH_UNARY_FUNC(FUNCTION, X)  \
    JSIMD_FALLBACK_MAPPING_LOOP(batch, FUNCTION(X[i]))

#define JSIMD_FALLBACK_BATCH_BINARY_FUNC(FUNCTION, X, Y)  \
    JSIMD_FALLBACK_MAPPING_LOOP(batch, FUNCTION(X[i], Y[i]))

#define JSIMD_FALLBACK_BATCH_TERNARY_FUNC(FUNCTION, X, Y, Z)  \
    JSIMD_FALLBACK_MAPPING_LOOP(batch, FUNCTION(X[i], Y[i], Z[i]))

// NOTE: Static casting a vector is static casting every element
#define JSIMD_FALLBACK_BATCH_STATIC_CAST(T_OUT, X)  \
...
#define JSIMD_FALLBACK_BOOL_CAST(T_OUT, X)  \
    ...
    namespace detail
    {
        ...
        struct batch_bool_kernel
        {
            ...
            {
                JSIMD_FALLBACK_BINARY_OP(batch_bool, &, lhs, rhs)
            }
            ...
            {
                JSIMD_FALLBACK_UNARY_OP(batch_bool, ~, rhs)
            }

            static batch_type bitwise_andnot(const batch_type& lhs, const batch_type& rhs)
            {
                JSIMD_FALLBACK_MAPPING_LOOP(batch_bool, (~(lhs[i] & rhs[i])))
            }
...
#undef FALLBACK_DEFINE_LOAD_STORE
    ...
    namespace detail
    {
        ...
        struct batch_kernel
        {
            ...
            {
                JSIMD_FALLBACK_UNARY_OP(batch, -, rhs)
            }

            static batch_type add(const batch_type& lhs, const batch_type& rhs)
            {
                JSIMD_FALLBACK_BINARY_OP(batch, +, lhs, rhs)
            }
            ...
            static batch_type bitwise_and(const batch_type& lhs, const batch_type& rhs)
            {
                JSIMD_FALLBACK_BATCH_BITWISE_BINARY_OP(&, lhs, rhs)
            }
            ...
            static batch_type bitwise_not(const batch_type& rhs)
            {
                JSIMD_FALLBACK_BATCH_BITWISE_UNARY_OP(~, rhs)
            }
            ...
            {
                JSIMD_FALLBACK_BATCH_BINARY_FUNC(std::min, lhs, rhs)
            }

            static batch_type max(const batch_type& lhs, const batch_type& rhs)
            {
                JSIMD_FALLBACK_BATCH_BINARY_FUNC(std::max, lhs, rhs)
            }

            static batch_type fmin(const batch_type& lhs, const batch_type& rhs)
            {
                JSIMD_FALLBACK_BATCH_BINARY_FUNC(std::fmin, lhs, rhs)
            }

            static batch_type fmax(const batch_type& lhs, const batch_type& rhs)
            {
                JSIMD_FALLBACK_BATCH_BINARY_FUNC(std::fmax, lhs, rhs)
            }

            static batch_type abs(const batch_type& rhs)
            {
                JSIMD_FALLBACK_BATCH_UNARY_FUNC(std::abs, rhs)
            }

            static batch_type fabs(const batch_type& rhs)
            {
                JSIMD_FALLBACK_BATCH_UNARY_FUNC(std::fabs, rhs)
            }

            static batch_type sqrt(const batch_type& rhs)
            {
                JSIMD_FALLBACK_BATCH_UNARY_FUNC(std::sqrt, rhs)
            }

            static batch_type fma(const batch_type& x, const batch_type& y, const batch_type& z)
            {
                JSIMD_FALLBACK_BATCH_TERNARY_FUNC(std::fma, x, y, z)
            }
            ...
            {
                JSIMD_FALLBACK_MAPPING_LOOP(batch, hadd(row[i]))
            }

            static batch_type select(const batch_bool_type& cond, const batch_type& a, const batch_type& b)
            {
                JSIMD_FALLBACK_MAPPING_LOOP(batch, (cond[i] ? a[i] : b[i]))
            }

            static batch_bool_type isnan(const batch_type& x)
            {
                JSIMD_FALLBACK_MAPPING_LOOP(batch_bool, std::isnan(x[i]))
            }
        };
    }
    ...
    inline batch<T, N> operator<<(const batch<T, N>& lhs, int32_t rhs) {
        JSIMD_FALLBACK_MAPPING_LOOP(batch, (lhs[i] << rhs))
    }
    ...
    {
        JSIMD_FALLBACK_BATCH_STATIC_CAST(int32_t, x)
    }

    template <std::size_t N>
    inline batch<int64_t, N> to_int(const batch<double, N>& x)
    {
        JSIMD_FALLBACK_BATCH_STATIC_CAST(int64_t, x)
    }

    template <std::size_t N>
    inline batch<float, N> to_float(const batch<int32_t, N>& x)
    {
        JSIMD_FALLBACK_BATCH_STATIC_CAST(float, x)
    }

    template <std::size_t N>
    inline batch<double, N> to_float(const batch<int64_t, N>& x)
    {
        JSIMD_FALLBACK_BATCH_STATIC_CAST(double, x)
    }
