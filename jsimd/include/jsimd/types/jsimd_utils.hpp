...
#define JSIMD_MACRO_UNROLL_BINARY(FUNC)                                                                   \
    ...
    // This is used in both jsimd_complex_base.hpp and jsimd_traits.hpp
    // However jsimd_traits.hpp indirectly includes jsimd_complex_base.hpp
    // so we cannot define is_complex in jsimd_traits.hpp. Besides, if
    // no file defining batches is included, we still need this definition
    // in jsimd_traits.hpp, so let's define it here.
