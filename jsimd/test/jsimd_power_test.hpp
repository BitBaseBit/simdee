#ifndef JSIMD_POWER_TEST_HPP
#define JSIMD_POWER_TEST_HPP
...
namespace jsimd
{
    ...
    bool test_simd_power(std::ostream& out, T& tester)
    {
        ...
        success = success && tmp_success;
        success &= check_almost_equal(topic, jsimd::pow(tester.lhs[0], - tester.lhs.size() / tester.size / 2),
        ...
        success = success && tmp_success;
        success &= check_almost_equal(topic, jsimd::hypot(tester.lhs[0], tester.rhs[0]), tester.hypot_res[0], out);
        ...
        success &= check_almost_equal(topic, jsimd::cbrt(tester.lhs[0]), res[0], out);
