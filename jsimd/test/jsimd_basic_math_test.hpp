#ifndef JSIMD_BASIC_MATH_TEST_HPP
#define JSIMD_BASIC_MATH_TEST_HPP
...
namespace jsimd
{
    ...
    bool test_simd_basic_math(std::ostream& out, T& tester)
    {
        ...
        success &= check_almost_equal(topic, jsimd::fmod(lhs[0], rhs[0]), tester.fmod_res[0], out);
        ...
        success = success && tmp_success;
        success &= check_almost_equal(topic, jsimd::remainder(lhs[0], rhs[0]), tester.remainder_res[0], out);
        ...
        success = success && tmp_success;
        success &= check_almost_equal(topic, jsimd::fdim(lhs[0], rhs[0]), tester.fdim_res[0], out);
        ...
        success = success && tmp_success;
        success &= check_almost_equal(topic, jsimd::clip(lhs[0], tester.clip_lo, tester.clip_hi), tester.clip_res[0], out);
        ...
        success = success && tmp_success;
        scalar_cond_res = jsimd::isfinite(lhs[0])?1.:0.;
        ...
        tmp_success = check_almost_equal(topic, res, tester.inf_res, out);
        success = success && tmp_success;
        scalar_cond_res = jsimd::isinf(lhs[0])?0.:1.;
        success &= check_almost_equal(topic, scalar_cond_res, tester.inf_res[0], out);
        ...
        success = success && tmp_success;
        success &= check_almost_equal(topic, jsimd::nextafter(lhs[0], rhs[0]), tester.nextafter_res[0], out);
