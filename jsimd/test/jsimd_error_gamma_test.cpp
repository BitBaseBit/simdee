...
#include "jsimd/math/jsimd_error.hpp"
#include "jsimd/math/jsimd_gamma.hpp"
#include "jsimd/memory/jsimd_aligned_allocator.hpp"
#include "jsimd/types/jsimd_types_include.hpp"
#include "jsimd_error_gamma_test.hpp"
...
TEST(jsimd, sse_float_error_gamma)
{
    std::ofstream out("log/sse_float_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<float, 4, 16>(out, "sse float");
    EXPECT_TRUE(res);
}

TEST(jsimd, sse_double_error_gamma)
{
    std::ofstream out("log/sse_double_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<double, 2, 16>(out, "sse double");
    EXPECT_TRUE(res);
}
#endif

#if JSIMD_X86_INSTR_SET >= JSIMD_X86_AVX_VERSION
TEST(jsimd, avx_float_error_gamma)
{
    std::ofstream out("log/avx_float_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<float, 8, 32>(out, "avx float");
    EXPECT_TRUE(res);
}

TEST(jsimd, avx_double_error_gamma)
{
    std::ofstream out("log/sse_float_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<double, 4, 32>(out, "avx double");
    EXPECT_TRUE(res);
}
#endif

#if JSIMD_X86_INSTR_SET >= JSIMD_X86_AVX512_VERSION
TEST(jsimd, avx512_float_error_gamma)
{
    std::ofstream out("log/avx512_float_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<float, 16, 64>(out, "avx512 float");
    EXPECT_TRUE(res);
}

TEST(jsimd, avx512_double_error_gamma)
{
    std::ofstream out("log/avx512_double_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<double, 8, 64>(out, "avx512 double");
    EXPECT_TRUE(res);
}
#endif

#if JSIMD_ARM_INSTR_SET >= JSIMD_ARM7_NEON_VERSION
TEST(jsimd, neon_float_error_gamma)
{
    std::ofstream out("log/neon_float_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<float, 4, 16>(out, "neon float");
    EXPECT_TRUE(res);
}
#endif
#if JSIMD_ARM_INSTR_SET >= JSIMD_ARM8_64_NEON_VERSION
TEST(jsimd, neon_double_error_gamma)
{
    std::ofstream out("log/neon_double_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<double, 2, 32>(out, "neon double");
    EXPECT_TRUE(res);
}
#endif

#if defined(JSIMD_ENABLE_FALLBACK)
TEST(jsimd, fallback_float_error_gamma)
{
    std::ofstream out("log/fallback_float_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<float, 7, 32>(out, "fallback float");
    EXPECT_TRUE(res);
}

TEST(jsimd, fallback_double_error_gamma)
{
    std::ofstream out("log/fallback_double_error_gamma.log", std::ios_base::out);
    bool res = jsimd::test_error_gamma<double, 3, 32>(out, "fallback double");
    EXPECT_TRUE(res);
}
#endif
