#pragma once
#ifndef JVEC_SIMD4X4F_H
  #include "JVec/simd4x4f.h"
#endif

#ifndef JVEC_VEC4F_H
  #include "JVec/vec4f.h"
#endif
...
namespace JVec {
    ...
    JVec_inline mat4f operator*(const mat4f& lhs, const mat4f& rhs) {
        mat4f ret;
        simd4x4f_matrix_mul(&lhs.value, &rhs.value, &ret.value);
        return ret;
    }

    JVec_inline mat4f operator*=(mat4f& lhs, const mat4f& rhs) {
    ...
    JVec_inline vec4f operator*(const mat4f& lhs, const vec4f& rhs) {
        vec4f ret;
        simd4x4f_matrix_vector_mul(&lhs.value, &rhs.value, &ret.value);
        return ret;
    }

    JVec_inline vec3f transformVector(const mat4f& lhs, const vec3f& rhs) {
        vec3f ret;
        simd4x4f_matrix_vector3_mul(&lhs.value, &rhs.value, &ret.value);
        return ret;
    }

    JVec_inline vec4f transformVector(const mat4f& lhs, const vec4f& rhs) {
        vec4f ret;
        simd4x4f_matrix_vector_mul(&lhs.value, &rhs.value, &ret.value);
        return ret;
    }
    
    JVec_inline vec3f transformPoint(const mat4f& lhs, const vec3f& rhs) {
        vec3f ret;
        simd4x4f_matrix_point3_mul(&lhs.value, &rhs.value, &ret.value);
        return ret;
    }

    JVec_inline vec3f orthoInverseTransformPoint(const mat4f& lhs, const vec3f& rhs) {
        vec3f ret;
        simd4x4f_inv_ortho_matrix_point3_mul(&lhs.value, &rhs.value, &ret.value);
        return ret;
    }

    JVec_inline vec3f orthoInverseTransformVector(const mat4f& lhs, const vec3f& rhs) {
        vec3f ret;
    ...
    JVec_inline mat4f transpose(const mat4f& m) {
        mat4f ret;
    ...
    JVec_inline mat4f inverse(const mat4f& m) {
        mat4f ret;
...
JVec_inline std::ostream& operator<<(std::ostream& os, const JVec::mat4f& v) {
