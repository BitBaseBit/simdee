...
#include "jsimd_hyperbolic.hpp"
#include "jsimd_logarithm.hpp"
#include "jsimd_power.hpp"
...
namespace jsimd
{
    ...
    namespace detail
    {
        ...
        inline batch<T, N> tan_complex_impl(const batch<T, N>& z)
        {
            ...
            b_type wres = select(jsimd::isinf(wimag), b_type(wreal, r_type(1.)), b_type(wreal, wimag / d));
    ...
    namespace detail
    {
        ...
        struct complex_batch_kernel
        {
            ...
            static batch_type sqrt(const batch_type& z)
            {
                ...
                real_batch x = z.real();
                real_batch y = z.imag();
                real_batch sqrt_x = jsimd::sqrt(fabs(x));
                real_batch sqrt_hy = jsimd::sqrt(0.5 * fabs(y));
                ...
                real_batch t0 = select(condxp, jsimd::sqrt(0.5 * (r + x)), jsimd::sqrt(0.5 * (r - x)));
            ...
            {
                return batch_bool_type(jsimd::isnan(z.real()) || jsimd::isnan(z.imag()));
            }
        };
    ...
    inline batch_type_t<X> proj(const simd_base<X>& rhs)
    {
        ...
        auto cond = jsimd::isinf(rhs().real()) || jsimd::isinf(rhs().imag());
