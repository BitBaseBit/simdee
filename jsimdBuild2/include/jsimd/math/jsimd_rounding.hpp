...
namespace jsimd
{
    ...
    namespace impl
    {
    ...
    #elif (JSIMD_X86_INSTR_SET >= JSIMD_X86_SSE2_VERSION) || (JSIMD_ARM_INSTR_SET == JSIMD_ARM7_NEON_VERSION)
    ...
    #if JSIMD_ARM_INSTR_SET >= JSIMD_ARM8_32_NEON_VERSION
        template <>
        inline batch<float, 4> ceil(const batch<float, 4>& x)
        {
            return vrndpq_f32(x);
        ...
        {
            JSIMD_FALLBACK_BATCH_UNARY_FUNC(std::ceil, x)
        }

        template <class T, std::size_t N>
        inline batch<T, N> floor(const batch<T, N>& x)
        {
            JSIMD_FALLBACK_BATCH_UNARY_FUNC(std::floor, x)
        }

        template <class T, std::size_t N>
        inline batch<T, N> trunc(const batch<T, N>& x)
        {
            JSIMD_FALLBACK_BATCH_UNARY_FUNC(std::trunc, x)
        }

        template <class T, std::size_t N>
        inline batch<T, N> nearbyint(const batch<T, N>& x)
        {
            JSIMD_FALLBACK_BATCH_UNARY_FUNC(std::nearbyint, x)
        }
    #endif
